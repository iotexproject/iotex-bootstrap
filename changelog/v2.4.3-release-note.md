...
To also correct already-recorded forged transaction logs on API/archive nodes, place the transaction-log patch file at `$IOTEX_HOME/data/txlog.db.patch` and add to the `chain` section of `config.yaml`:
...
Then restart the node. Applying the patch is optional and only affects the transaction logs served for the patched heights; it does not change balances, receipts, or block hashes.
