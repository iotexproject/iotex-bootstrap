...
|---|---|---|---|---|
| TestNet | 46880641 | 47141281 | 47468161 | activated 2026-08-21 / 08-29 / 09-07 |
| MainNet | **53155801** | **53155801** | **53155801** | estimated 2026-10-08 02:00 UTC |
...
### What MainNet delegates should do before 53155801
...
|---------|--------|--------------|
| TestNet | 46880641 | 2026-08-21 ~02:00 UTC |
| MainNet | 53155801 | estimated 2026-10-08 02:00 UTC |
...
| Network | Height | Approx. date |
|---------|--------|--------------|
| TestNet | 47141281 | 2026-08-28 ~22:00 UTC |
| MainNet | 53155801 | estimated 2026-10-08 02:00 UTC — same block as Zanzibar |
...
| Network | Height | Approx. date |
|---------|--------|--------------|
| TestNet | 47468161 | activated 2026-09-07 10:00 UTC |
| MainNet | 53155801 | estimated 2026-10-08 02:00 UTC — same block as Zanzibar |
...
Confirm the node reports `v2.5.0` before block 53155801. There is no genesis
file to verify, which removes the ordering trap the TestNet rollout had.

### TestNet

The TestNet rollout runs the **`v2.5.0-rc2`** pre-release. Nodes on `rc1` must
upgrade before using the current genesis because `rc1` does not know about
Zanzibar Gamma.
...
Then restart your node against the `iotex/iotex-core:v2.5.0-rc2` image. Or non-interactively:

```bash
bash setup_fullnode.sh --auto --home=$IOTEX_HOME --version=v2.5.0-rc2
```

Pull the image and restart **before** replacing the genesis: `rc1` cannot parse
a genesis containing `zanzibarGammaHeight`, and will fail to start if it is
swapped in first with:

```
failed to unmarshal yaml genesis to struct: yaml: unmarshal errors:
  field zanzibarGammaHeight not found in type genesis.Blockchain
```

Confirm the node picked up the current genesis. Check that it contains
`zanzibarGammaHeight: 47468161` together with `zanzibarBetaHeight: 47141281`
